...
class Customers
{
	...
	public function getCustomers(){
		$query = $this->con->query("SELECT `user_id`, `first_name`, `last_name`, `mobile`, `username`, `epfno` FROM `android_app_users`");
		$ar = [];
		if (@$query->num_rows > 0) {
	...
	public function deleteCustomers($uid = null){
		if ($uid != null) {
			$q = $this->con->query("DELETE FROM android_app_users WHERE user_id = '$uid'");
			if ($q) {
